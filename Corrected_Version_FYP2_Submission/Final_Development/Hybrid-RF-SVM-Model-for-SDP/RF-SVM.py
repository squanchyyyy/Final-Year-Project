...
# 5️⃣ Train Base Models (Random Forest & SVM)
...
# 6️⃣ Get Probabilities from Base Models
rf_proba = rf_model.predict_proba(X_test_scaled)[:, 1]
svm_proba = svm_model.predict_proba(X_test_scaled)[:, 1]

# 7️⃣ Stack Predictions as Features for Meta-Model (Logistic Regression)
stacked_features = np.column_stack((rf_proba, svm_proba))
meta_model = LogisticRegression()
meta_model.fit(stacked_features, y_test)

# 8️⃣ Evaluate Meta-Model
...
# 9️⃣ Predict Total Defects in Full Dataset
X_scaled_full = scaler.transform(X)
rf_full_proba = rf_model.predict_proba(X_scaled_full)[:, 1]
svm_full_proba = svm_model.predict_proba(X_scaled_full)[:, 1]
full_stacked = np.column_stack((rf_full_proba, svm_full_proba))

final_full_proba = meta_model.predict_proba(full_stacked)[:, 1]
final_full_pred = (final_full_proba >= 0.5).astype(int)

# 🔢 Breakdown of predicted vs. actual counts
predicted_non_defective = np.sum(final_full_pred == 0)
predicted_defective = np.sum(final_full_pred == 1)
...
5. TRAIN Base Models (Random Forest and SVM)

6. STACK Predictions from RF and SVM → input for meta-model
...
9. PREDICT FULL DATASET
   ...
   - Predict defect probabilities using meta-model
